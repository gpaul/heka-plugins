...
func UnixInputSpec(c gs.Context) {
	...
	c.Specify("A UnixInput", func() {
		...
		c.Specify("using a unix address", func() {
			ith.AddrStr = "@/heka/unix-test"
			ith.ResolvedAddrStr = "@/heka/unix-test"
			config.Net = "unix"
			config.Address = ith.AddrStr

			err := udpInput.Init(config)
			c.Assume(err, gs.IsNil)
			realListener := (unixInput.listener).(*net.UnixConn)
			c.Expect(realListener.LocalAddr().String(), gs.Equals, ith.ResolvedAddrStr)

			c.Specify("passes the connection to SplitStream", func() {
				go unixInput.Run(ith.MockInputRunner, ith.MockHelper)

				conn, err := net.Dial("unix", ith.AddrStr)
				c.Assume(err, gs.IsNil)
...
func UnixInputSpecFailure(c gs.Context) {
	unixInput := UnixInput{}
	err := unixInput.Init(&UnixInputConfig{
		Net:     "tcp",
