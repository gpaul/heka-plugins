...
type UnixCapnProtoInput struct {
	listener net.Listener
	wg       sync.WaitGroup
	stopChan chan bool
	ir       InputRunner
	config   *UnixCapnProtoInputConfig
}

type UnixCapnProtoInputConfig struct {
...
func (t *UnixCapnProtoInput) ConfigStruct() interface{} {
	config := &UnixCapnProtoInputConfig{
...
func (t *UnixCapnProtoInput) Init(config interface{}) error {
	var err error
	t.config = config.(*UnixCapnProtoInputConfig)
...
func (t *UnixCapnProtoInput) handleConnection(conn net.Conn) {
	...
	for !stopped {
		...
		default:
			...
			_, err := capn.ReadFromStream(conn, segmentBuf)
			if err != nil {
			...
			copy(record, b)
			sr.DeliverRecord(record, deliverer)
		}
	}
}

func (t *UnixCapnProtoInput) Run(ir InputRunner, h PluginHelper) error {
	t.ir = ir
	var conn net.Conn
	var e error
	for {
...
func (t *UnixCapnProtoInput) Stop() {
...
func init() {
	RegisterPlugin("UnixCapnProtoInput", func() interface{} {
		return new(UnixCapnProtoInput)
	})
}
